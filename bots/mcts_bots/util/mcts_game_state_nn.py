...
from .predict_trump_with_nn import TrumpPredictor
...
class MCTSGameStateNN(GameState):
    """
    ...
    def perform_action(self, action) -> MCTSGameStateNN:
        """
    ...
    def _action(self, action: int):
        if action < TRUMP_FULL_OFFSET:
            ...
            self._action_trump_nn(full_to_trump(action))
        self.update_legal_actions()

    def _action_trump_nn(self, action: int):
        """
        Chooses trump for the forehand player using the nn model’s recommendation.
        """
        if self.forehand == -1:
            ...
            # process the player hand, so that the nn model can use it
            processed_hand = self.preprocessor.preprocess_hand(self.hands[self.player])

            # Using external predict_trump function to get trump
            suggested_trump = self.predictor.predict(processed_hand)
            action = suggested_trump
            self.forehand = 1
    ...
    @classmethod
    def random_state_from_obs(cls, obs: GameObservation) -> MCTSGameStateNN:
        """
